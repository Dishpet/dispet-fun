/**
 * Marketing Ponuda v2 — 3D merch stage.
 *
 * A pinned scroll track: real shop models enter one at a time (spin in,
 * zoom to camera, glitch-swap their design, spin out), riding above a
 * tilted, clickable photo-gallery underlay. All choreography derives
 * from a single scroll progress value.
 */
...
import { GalleryLightbox, shopUrl, trackEvent } from "./shared";

/* ================================================================== */
/*  Design assets — the same collections the Shop page uses            */
/* ================================================================== */
...
/* ================================================================== */
/*  Act sequence — one product on stage at a time.                     */
...
interface ActSpec {
    ...
    price: string;
    targetHeight: number;
    /** zone that carries the two cycling designs */
    designZone: Zone;
    /** required facing while on stage (apparel shows its back) */
    faceBack: boolean;
    /** two unique designs for this act */
    designs: [string, string];
    /** body colors for design A/B (tshirt+hoodie only — shop palette) */
    colors?: [string, string];
    staticColor?: string;
    /** entry/exit positions (x, y) and spin direction multipliers */
...
/* "Made with Dišpet" — three phrases typed out one after another,
   character by character, spanning the whole show. */
const PHRASES = [
...
/* ================================================================== */
/*  Texture mapping (copied 1:1 from ShopScene tuning)                 */
/* ================================================================== */
...
/** Digital glitch on print areas — simplified port of the shop transition. */
const injectGlitch = (mat: THREE.MeshStandardMaterial) => {
    mat.userData.uniforms = { uGlitch: { value: 0 }, uTime: { value: 0 } };
...
/** Holographic vertical swipe for body-color changes (tshirt/hoodie). */
const injectColorSwipe = (mat: THREE.MeshStandardMaterial, fromColor: string) => {
    mat.userData.uniforms = {
        uRevealProgress: { value: 1.0 },
        uOldColor: { value: new THREE.Color(fromColor) },
        uNewColor: { value: new THREE.Color(fromColor) },
        uTime: { value: 0 },
        uModelHeight: { value: 2.0 },
        uModelMinY: { value: -1.0 },
    };
    mat.onBeforeCompile = (shader) => {
        for (const key of ["uRevealProgress", "uOldColor", "uNewColor", "uTime", "uModelHeight", "uModelMinY"]) {
        ...
        shader.fragmentShader = shader.fragmentShader
            ...
            .replace(
                ...
                `#include <color_fragment>
                float normalizedY = clamp((vWorldPos.y - uModelMinY) / uModelHeight, 0.0, 1.0);
                ...
                diffuseColor.rgb = mix(uNewColor, uOldColor, blend);
                float edgeDist = abs(normalizedY - cutoff);
                if (edgeDist < edgeWidth && uRevealProgress > 0.01 && uRevealProgress < 0.99) {
                    float edgeIntensity = pow(1.0 - (edgeDist / edgeWidth), 1.5);
                    vec3 rainbow = hsl2rgb_holo(fract(normalizedY * 2.0 + uTime * 0.5), 1.0, 0.6);
                    float scanline = pow(sin(normalizedY * 80.0 + uTime * 10.0) * 0.5 + 0.5, 4.0);
...
const ActModel = ({ act, index, progress, onLoaded }: {
    ...
    // Unblock the next model in the loading queue once this one is committed
    useEffect(() => {
    ...
    /** Clone the scene once; collect body/print materials and normalize size. */
        ...
        cloned.traverse((child) => {
            ...
            const remap = (m: THREE.Material) => {
                ...
                if (name.includes("print")) {
                    ...
                    mesh.renderOrder = 1;
                    injectGlitch(mat);
                    (zone === act.designZone ? designMats : logoMats).push(mat);
                } else {
                    // body — the bottle's BlackRing keeps its original look
                    if (!(mat.name || "").toLowerCase().includes("blackring")) {
                        bodyMats.push(mat);
                        mat.transparent = false;
                        mat.depthWrite = true;
                        mat.alphaTest = mat.map || mat.alphaMap ? 0.5 : 0;
                        if (act.colors) injectColorSwipe(mat, act.colors[0]);
                    }
    ...
    useFrame((state) => {
        ...
        const t = (progress.get() - segStart) / segLen; // local act time
        const onStage = t > -0.03 && t < 1.03;
        g.visible = onStage;
        if (!onStage) return;
        const tc = clamp01(t);

        /* Choreography: spin in (0→0.22) · zoom for the switch (0.3→0.45)
           · spin out (0.78→1). All scrubbed from scroll. */
        ...
        g.position.y = act.enterDir[1] * (1 - inT) + act.exitDir[1] * outT + Math.sin(tc * Math.PI * 2.5) * 0.05;
        ...
        /* Design switch, masked by the glitch */
        const glitch = bell(tc, act.switchAt, 0.12);
        const designIdx = tc < act.switchAt ? 0 : 1;
        if (designIdx !== lastDesignIdx.current) {
            lastDesignIdx.current = designIdx;
        ...
        /* Body color rides the same switch via the holographic swipe */
        if (act.colors) {
            g.updateMatrixWorld(true);
            tmpBox.setFromObject(g);
            const w = 0.06;
            const reveal = clamp01((tc - (act.switchAt - w)) / (2 * w));
            bodyMats.forEach((m) => {
                const u = m.userData.uniforms;
                if (!u) return;
                u.uModelHeight.value = tmpBox.max.y - tmpBox.min.y;
                u.uModelMinY.value = tmpBox.min.y;
                u.uRevealProgress.value = reveal;
                u.uTime.value = time;
                u.uOldColor.value.set(act.colors![0]);
                u.uNewColor.value.set(act.colors![1]);
            });
...
/* ================================================================== */
/*  Stage — sequential model loading, fits any viewport                */
/* ================================================================== */
...
const Stage = ({ progress }: { progress: MotionValue<number> }) => {
    ...
    return (
        <group scale={s} position={[0, -0.1, 0]}>
            {ACTS.map((act, i) => {
                // one model at a time keeps mobile GPUs from spiking
                if (i > 0 && !loaded.has(ACTS[i - 1].id)) return null;
                return (
                    <Suspense key={act.id} fallback={null}>
                        <ActModel act={act} index={i} progress={progress} onLoaded={() => markLoaded(act.id)} />
                    </Suspense>
                );
            })}
        </group>
...
/* ================================================================== */
/*  Canvas error boundary — a WebGL crash must never take the page     */
/* ================================================================== */
...
class CanvasErrorBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
    ...
    componentDidCatch(error: Error) {
        console.error("[MerchStage] 3D canvas crashed:", error);
    }
    render() {
        if (this.state.failed) {
            return (
                <div className="flex h-full items-center justify-center">
                    <p className="mk-display text-sm uppercase tracking-[0.3em] text-white/50">
                        3D prikaz nije dostupan
                    </p>
                </div>
            );
        }
...
const GalleryRow = ({
    ...
    // gentle travel across the whole pinned track
    ...
    return (
        <div className="flex overflow-hidden py-1 md:py-2">
            <motion.div className="flex flex-nowrap gap-3 md:gap-5" style={{ x }}>
                {doubled.map((src, i) => {
...
const Typewriter = ({ phase, chars }: { phase: number; chars: number }) => (
    <div className="relative min-h-[8rem]">
        {PHRASES.map((ph, i) => {
            const words = ph.text.split(" ");
            let charIdx = 0;
            return (
                <p key={i} className={`absolute inset-x-0 top-0 ${ph.cls} ${phase === i ? "" : "invisible"}`} aria-hidden>
                    {words.map((word, wi) => {
...
/* ================================================================== */
/*  Public component — the pinned track                                */
/* ================================================================== */

export const MerchStage = () => {
    ...
    /* Act + typewriter state, both derived purely from scroll position */
        ...
        if (p > PHRASE_WINDOWS[PHRASE_WINDOWS.length - 1][1]) {
            ...
            for (let k = 0; k < PHRASE_WINDOWS.length; k++) {
                ...
                if (p >= a && p <= b) {
                    ...
                    const reveal = Math.min(1, (p - a) / (b - a) / 0.35);
    ...
    return (
        <div ref={trackRef} className="relative h-[340vh]">
            <div className="sticky top-0 flex h-screen flex-col overflow-hidden">
                ...
                {/* LAYER 0 — tilted photo-gallery underlay; the middle stays open
                ...
                >
                    <GalleryRow photos={PHOTOS.slice(0, 16)} progress={scrollYProgress} direction="left" offset={0} onPhotoClick={openPhoto} />
                    <GalleryRow photos={PHOTOS.slice(16)} progress={scrollYProgress} direction="right" offset={5} onPhotoClick={openPhoto} />
                </div>
                ...
                >
                    ...
                    {/* Made with Dišpet — typed out phrase by phrase beside the model */}
                    <motion.div
                    ...
                    >
                        <Typewriter phase={type.phase} chars={type.chars} />
                    </motion.div>

                    {/* Act labels — crossfade with each product, deep-link to the shop */}
            ...
            <GalleryLightbox photos={PHOTOS} selected={selected} setSelected={setSelected} />
        </div>
    );
};
